...
(function () {
  ...
  var MANIFEST = {
    ...
    enemyOztrich:            { file: 'oztrich.glb',       height: 2.6 },
    enemyOztrichChase:       { file: 'oztrich_chase.glb', height: 2.6 },
    enemyPidgin:             { file: 'pidgin.glb',        height: 1.4 },
    enemyPidginChase:        { file: 'pidgin_chase.glb',  height: 1.4 },
    enemySeagle:             { file: 'seagle.glb',        height: 1.5 },
    enemySeagleChase:        { file: 'seagle_chase.glb',  height: 1.5 },
  ...
  function bootstrapLoader() {
    ...
    return fetchText(CDN + 'utils/BufferGeometryUtils.js').then(function (bgu) {
      ...
      return fetchText(CDN + 'loaders/GLTFLoader.js').then(function (src) {
        ...
        return import(url).then(function (mod) { return new mod.GLTFLoader(); });
      });
    });
  }
  ...
  function normalize(sceneRoot, spec) {
    ...
    wrap.traverse(function (o) {
      if (o.isMesh) {
        ...
        if (o.isSkinnedMesh && !skinWarned) {
          skinWarned = true;
          diag('warn', 'a model contains a skinned rig — it will display statically (rig animation is a later phase)');
        }
      }
    });
    return wrap;
  ...
  A.boot = function () {
    ...
    bootstrapLoader().then(function (l) {
      loader = l;
      diag('log', 'official GLTFLoader ready (bootstrapped onto global THREE)');
      var all = Object.keys(MANIFEST);
      var deferred = all.filter(function (k) { return MANIFEST[k].swap === false; });
      var keys = all.filter(function (k) { return MANIFEST[k].swap !== false; });
      ...
      keys.forEach(function (key) {
        ...
        loader.load(PATH + spec.file, function (gltf) {
          try {
            cache[key] = normalize(gltf.scene, spec);
            okC++; diag('log', 'GLB loaded: ' + key + ' ← ' + spec.file);
          } catch (e) { missC++; diag('warn', 'GLB normalize failed for ' + key + ': ' + (e && e.message)); }
          if (++done === keys.length) summary();
        }, undefined, function () {
          missC++;                        // missing file: normal, placeholder used
          if (++done === keys.length) summary();
        });
      });
      function summary() {
